...
#define MAX_NUM_OF_SECONDS                                                     \
  (5 * 365 * 24 * 60 * 60) // number of seconds in 5 years
...
void TimeDaemon::calculateDrift() {
  ...
  iAdjustedDaemonTime = iAverage + sDaemonTime;
}

void TimeDaemon::setDaemonTime() { 
  // this->sDaemonTime = sTime;
  struct timeval master_time;
  gettimeofday(&master_time, NULL);
  int master_msec = master_time.tv_usec;
  this->sDaemonTime = master_msec;
}
...
bool TimeDaemon::readInputFile(string sFile) {
  ...
  try {
    ...
    std::cout << "\nException thrown while reading client transactions in "
              << __func__;
    res = false;
  }
...
int TimeDaemon::startMaster() {
  ...
  for (int i = 0; i < vSockets.size(); i++) {
    Socket::sendData(vSockets[i], std::to_string(sDaemonTime));
    string sClient;
