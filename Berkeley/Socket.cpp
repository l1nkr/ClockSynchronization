...
bool Socket::bind(SocketData &s) {
  ...
  int ret =
      ::bind(s.iSocket, (struct sockaddr *)&s.sockAddr, sizeof(s.sockAddr));
  // std::cout << "\nbind_returned: " << ret;
  if (ret == -1) {
    cout << "\nError in " << __func__;
    res = false;
...
bool Socket::accept(SocketData &oldSock, SocketData &newSock) {
  ...
  newSock.iSocket = ::accept(oldSock.iSocket, (sockaddr *)&oldSock.sockAddr,
...
bool Socket::setSocketOptions(SocketData &s) {
  bool res = true;
  int on = 1;
  if (setsockopt(s.iSocket, SOL_SOCKET, SO_REUSEADDR, (const char *)&on,
                 sizeof(on)) == -1) {
    cout << "\nError in " << __func__;
    res = false;
  }
...
bool Socket::connect(SocketData &s) {
  ...
  status =
      ::connect(s.iSocket, (struct sockaddr *)&s.sockAddr, sizeof(s.sockAddr));
...
bool Socket::isValidSocket(int &sockFd) { return (sockFd != -1); }
